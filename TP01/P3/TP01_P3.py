...
Uso:
    python TP01_P3.py <dir_coleccion> [--stopwords <archivo_stopwords>] [--stats <collection_data.json>]
...
def is_valid_term(term: str, token_type: str, stopwords: set) -> bool:
    """
    Filtra un término por longitud y stopwords.
    URLs y emails quedan exentos del límite MAX_TERM_LEN.
    """
    ...
    if len(sys.argv) < 2:
        ...
        if "--stats" in sys.argv:
            ...
            if idx + 1 < len(sys.argv):
                ...
                print("[ERROR] Se indicó --stats pero no se proporcionó el archivo.")
                sys.exit(1)

    return dir_path, stopwords_file, stats_file
...
def process_collection(dir_path: str, stopwords: set):
    ...
    for filename in files:
        ...
        # Tokenización → lista de (token, tipo)
...
def main():
    dir_path, stopwords_file, stats_file = parse_args()
    ...
    # -------------------------------------------------------------------------------
    # Comparar con stats (solo si se pasó --stats)
    if stats_file:
        if not isfile(stats_file):
            print(f"[ERROR] El archivo de stats '{stats_file}' no existe.")
        else:
            ...
            if stats:
                print(f"[INFO] Cantidad de tokens: {stats.get('num_tokens', 'N/A')}")
                print(f"[INFO] Cantidad de términos: {stats.get('num_terms', 'N/A')}")
                print(f"[INFO] Cantidad de documentos procesados: {stats.get('N', 'N/A')}\n")
            ...
            for data in coll_data.get("data", []):
                ...
                if expected_df == current_df:
                    ...
                    failures += 1
                    print(f"[INFO] Falla en el termino '{term}': DF calculado = {current_df} | DF esperado = {expected_df}")

            if failures == 0:
                print(f"[INFO] Todos los {coincidences} terminos comparados coinciden exitosamente con su respectivo DF.")
            else:
                print(f"[INFO] Terminos comparados exitosamente: {coincidences}. Fallas: {failures}.")
